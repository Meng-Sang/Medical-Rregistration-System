...
$(function () {
    $("#name").blur(function()
    {
        var $Tip = $("#nameTip span");
        if(!/^[\u4e00-\u9fa5]{2,20}$/.test($(this).val()))
        {
            $Tip.text("请输入正确的姓名");
        }
        else
    ...
    {
        var $Tip = $("#idTip span");
        if(!/^[1-9]\d{5}(18|19|20)\d{2}((0[1-9])|(1[0-2]))(([0-2][1-9])|10|20|30|31)\d{3}[0-9Xx]$/.test($(this).val()))
        {
            $Tip.text("请输入正确的身份证号");
        }
        else
...
{
    if($("#name").val()!=""&&$("#nameTip span").text()==""
        &&$("#id").val()!=""&&$("#idTip span").text()==""
        &&$("#s1").val()!=""&&$("#s2").val()!=""&&$("#pcTip span").text()==""
        &&$("#address").val()!=""&&$("#addressTip span").text()==""
        &&$("#date").val()!=""&&$("#dateTip span").text()==""
    )
    {
        if(confirm("信息一旦提交将无法进行更改，是否进行提交！！！"))
        {
        return true;
        }
        else
